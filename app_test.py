...
if run_btn and uploaded is not None:
    ...
    if chart_data:
        ...
        for d in chart_data:
            rows.append({
                "Organ":             d["organ"],
                "Control (mean RE)": f"{d['ctrl_mean']:.4f}",
                "Drug (mean RE)":    f"{d['drug_mean']:.4f}",
                "Control group":     d["ctrl_label"],
                "Drug group(s)":     ", ".join(d["drug_labels"]),
            })
    ...
    with st.expander("🔍 Detected data blocks (debug info)"):
        for blk in output["blocks"]:
            st.markdown(f"""
**{blk['organ']}**
- Control group: `{blk['ctrl_label']}` — rows {blk['ctrl_rows']}
- Drug group(s): `{', '.join(blk['drug_labels'])}` — rows {blk['drug_rows']}
            """)
